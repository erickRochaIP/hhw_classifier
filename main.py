...
model = nhw.Net()
model.load_state_dict(torch.load('./models/model002.pth'))
# Colors
BLACK = (0, 0, 0)
WHITE = (255, 255, 255)
...
# Start pygame
pygame.init()
pygame.display.set_caption('Hebrew Handwriting Classifier')
...
while True:
    ...
    for i in range(ROWS):
        ...
        for j in range(COLS):
            ...
            if mouse and rect.collidepoint(mouse):
                ...
                if i + 1 < ROWS:
                    handwriting[i + 1][j] = 250 / 255
                if j + 1 < COLS:
                    handwriting[i][j + 1] = 250 / 255
                if i + 1 < ROWS and j + 1 < COLS:
                    handwriting[i + 1][j + 1] = 250 / 255
    ...
    if mouse and classifyButton.collidepoint(mouse):
        classification = nhw.classify(
            np.array(handwriting),
            model
        )
    ...
    discText = smallFont.render("letters available: shin, bet", True, WHITE)
    discRect = discText.get_rect()
    grid_size = OFFSET * 4 + CELL_SIZE * COLS
    discRect.center = (
        grid_size + ((width - grid_size) / 3),
        300
    )
    screen.blit(discText, discRect)
